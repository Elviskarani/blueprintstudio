...
const ServiceItem: React.FC<ServiceItemProps> = ({ number, title }) => {
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 bg-gray-800 transition-transform duration-300 ${
        isHovered ? 'translate-x-0' : '-translate-x-full'
      }`} />

      <div className="flex items-center gap-4 relative z-10">
        <span className={`text-sm font-medium transition-colors duration-300 ${
          isHovered ? 'text-lime-400' : 'text-gray-500'
        }`}>{number}</span>
      ...
      <div className="relative z-10 flex items-center gap-2">
        <ArrowUpRight 
          className={`transform transition-all duration-300 ${
            isHovered ? 'opacity-100 translate-x-0 text-white' : 'opacity-0 -translate-x-4'
          }`} 
          size={20} 
        />
        <Plus 
        ...
        <div className="flex items-center justify-between relative z-10">
          ...
          <div className={`rounded-full p-2 transition-all duration-300 ${
            ...
            <ArrowRight 
              size={20} 
              className={`transform transition-all duration-300 ${
                isHovered ? 'translate-x-1 text-lime-400' : isDark ? 'text-gray-900' : 'text-white'
              }`}
            />
          </div>
        </div>
